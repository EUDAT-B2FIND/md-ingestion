...
class ArcGISHarvester(Harvester):
    ...
    def query(self):
        if not self._query:
            self._query = "where=+lokalid%3D%2722228%27&geometryType=esriGeometryEnvelope&spatialRel=esriSpatialRelIntersects&outFields=*&returnGeometry=true&returnTrueCurves=false&returnIdsOnly=false&returnCountOnly=false&returnZ=false&returnM=false&returnDistinctValues=false&resultOffset=&resultRecordCount=&returnExtentOnly=false&f=geojson"  # noqa
        return self._query
