...
def test_pangaea():
    xmlfile = os.path.join(TESTDATA_DIR, 'pangaea', 'raw', 'ef848f90-295c-54e6-b822-c0b66f8bec64.xml')
...
def test_pangaea_temporal():
    xmlfile = os.path.join(TESTDATA_DIR, 'pangaea', 'raw', 'e4723633-ab46-5316-875f-fcd788dc0931.xml')
    reader = PangaeaDatacite()
    doc = reader.read(xmlfile)
    # assert 'https://doi.pangaea.de/10.1594/PANGAEA.904099' == doc.doi
    # assert '' == doc.source
...
def test_pangaea_bbox():
    xmlfile = os.path.join(TESTDATA_DIR, 'pangaea', 'raw', 'c1d81bf1-758f-5b85-a93f-17827fce4900.xml')
    reader = PangaeaDatacite()
    doc = reader.read(xmlfile)
...
def test_pangaea_handle():
    xmlfile = os.path.join(TESTDATA_DIR, 'pangaea', 'raw', '7c0209b8-6189-5bda-89eb-7d5231d6e72d.xml')
    reader = PangaeaDatacite()
    doc = reader.read(xmlfile)
...
def test_pangaea_keywords():
    xmlfile = os.path.join(TESTDATA_DIR, 'pangaea', 'raw', 'ee3796e4-4e01-5e4e-b2d8-ae67ab4b7381.xml')
    reader = PangaeaDatacite()
    doc = reader.read(xmlfile)
    assert 'https://doi.org/10.1594/PANGAEA.889006' == doc.doi
    assert ['EPD', 'DEPTH', 'sediment rock', 'Age', 'comment', 'dated standard deviation', 'Calendar age', 'maximum old', 'minimum young', '²¹⁰Pbₓ ¹³⁷Cs Lead Caesium', 'calibrated'] == doc.keywords  # noqa
